...
'''
Author: user
Date: 2022-04-04 10:28:48
LastEditors: user
LastEditTime: 2022-04-12 20:52:01
Descripttion: 
'''
import pandas as pd
import numpy as np
...
# # !标准化
# datagroup_shijianchuo['If_Time_chuo']=datagroup_shijianchuo.If_Time_chuo.apply(lambda x : (x-np.min(datagroup_shijianchuo.If_Time_chuo))/(np.max(datagroup_shijianchuo.If_Time_chuo) - np.min(datagroup_shijianchuo.If_Time_chuo))*6000)
...
# 9.1、分类
sort_after_lof_data.loc[sort_after_lof_data['z_score']>= sort_after_lof_data.iloc[380, 5], 'class'] = 0
sort_after_lof_data.loc[sort_after_lof_data['z_score'] < sort_after_lof_data.iloc[380, 5], 'class'] = 1
# 9.2、绘图
plt.title("LOF")
plt.scatter(sort_after_lof_data['times'][:380],sort_after_lof_data['aver'][:380], color='r', label="inliner")
plt.scatter(sort_after_lof_data['times'][380:],sort_after_lof_data['aver'][380:], color='g', label="outliner")
plt.legend()
plt.show()
print("9、处理相关值并且绘图\n")