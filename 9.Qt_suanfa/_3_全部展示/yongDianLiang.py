...
'''
Author: user
Date: 2022-04-13 22:51:48
LastEditors: user
LastEditTime: 2022-04-14 17:54:44
Descripttion: 
'''
#!/usr/bin/env python
# coding=UTF-8
...
def data_chuli_yongdianliang(filename_day=r"D:\1_kaoyan\college_ending\400仪表数据--全\middata - 副本\日结用电量数据——按照不同类型的功率收费区分\处理过的\FourHundred_daily_P0300.csv"):
...
def draw_gulisenlin_zhifangtu(data_guli):
    ...
    plt.hist(data_guli[data_guli['label']==1].scores,color='r',label='正常值')
    plt.hist(data_guli[data_guli['label']==-1].scores,color='steelblue',label='异常值')
...
def draw_gulisenlin_sanDianTu(data_guli):
    ...
    ax.scatter(data_guli[data_guli['label']==1].sum_all, data_guli[data_guli['label']==1].aver_month,data_guli[data_guli['label']==1].std_month,c='r',marker='o',label="正常值")
    ax.scatter(data_guli[data_guli['label']==-1].sum_all, data_guli[data_guli['label']==-1].aver_month,data_guli[data_guli['label']==-1].std_month,c='b',marker='^',label="异常值")
    plt.title("孤立森林正常值和异常值的参数（用电量）三维散点图")
    plt.xlabel("每年用电量")
    plt.ylabel("每月平均用电量")
    plt.ylabel("每月用电量的标准差")
    plt.legend(loc='best',fontsize=10)
    plt.show()